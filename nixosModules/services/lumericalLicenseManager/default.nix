...
}:
{
  ...
  config =
    let
      inherit (config.nixos.services) lumericalLicenseManager;
    in
    lib.mkIf (lumericalLicenseManager != null) {
      virtualisation.oci-containers.containers.lumericalLicenseManager = {
        ...
        volumes =
          ...
          in
          [ "${license}:/home/ansys_inc/shared_files/licensing/license_files/ansyslmd.lic" ];
      };
