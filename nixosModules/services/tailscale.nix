inputs: {
  ...
  config =
    let
      inherit (inputs.config.nixos.services) tailscale;
    in
    inputs.lib.mkIf (tailscale != null) {
      services.tailscale = {
        enable = true;
        openFirewall = true;
        ...
        extraUpFlags = [
          "--login-server=https://headscale.chn.moe"
          "--accept-dns=false"
          "--netfilter-mode=off"
        ];
        extraSetFlags = [
      ...
      users = {
        users.tailscale = {
          uid = inputs.config.nixos.user.uid.tailscale;
          group = "tailscale";
          isSystemUser = true;
        };
