...
}:
{
  options.nixos.services.slurm =
    ...
    in
    {
      ...
      master = mkOption {
        ...
        default = config.nixos.model.hostname;
      };
      node = mkOption {
        type = types.attrsOf (
  ...
  config =
    let
      inherit (config.nixos.services) slurm;
    in
    lib.mkIf slurm.enable (
      let
        info = pkgs.localPkgs.info.override {
        ...
        # worker 配置
        {
          services = {
            slurm = {
                  ...
                  gpuString =
                    if node.value.gpus == null then
                      ...
                      + builtins.concatStringsSep "," (
                        builtins.map (gpu: "gpu:${gpu.name}:${builtins.toString gpu.value}") (
              ...
              extraConfigPaths =
                let
                  gpus = slurm.node.${config.nixos.model.hostname}.gpus or null;
                in
                lib.mkIf (gpus != null) (
                  let
                    gpuString = builtins.concatStringsSep "\n" (
                      builtins.map (gpu: "Name=gpu Type=${gpu.name} Count=${builtins.toString gpu.value}") (
                        lib.attrsToList gpus
                      )
                    );
                  in
                  [ (pkgs.writeTextDir "gres.conf" "AutoDetect=nvml\n${gpuString}") ]
                );
              extraCgroupConfig = ''
                ConstrainCores=yes
            ...
            munge = {
              enable = true;
              password = config.nixos.system.sops.secrets."slurm/munge".path;
            };
          };
          ...
          nixos.system.sops = {
            ...
            // builtins.listToAttrs (
              builtins.map (n: lib.nameValuePair "telegram/${n}" { }) [
                "token"
                "user/chn"
                "user/hjp"
                "user/root"
              ...
              content =
                ...
                builtins.toJSON {
                  ...
                  user = builtins.listToAttrs (
                    builtins.map (n: lib.nameValuePair n placeholder."telegram/user/${n}") [
                      "chn"
                      "hjp"
                      "root"
                    ]
        ...
        (lib.mkIf (slurm.master == config.nixos.model.hostname) {
          ...
          nixos = {
            user.sharedModules = [
              {
                home.packages = [
                  (pkgs.writeShellScriptBin "sbatch" ''if [ "$#" -eq 0 ]; then sbatch-tui; else /run/current-system/sw/bin/sbatch "$@"; fi'')
                ];
              }
            ];
            services.mariadb = {
            ...
            system.sops.secrets."slurm/db" = {
              owner = "slurm";
              key = "mariadb/slurm";
            };
          };
          environment.systemPackages = [
            (pkgs.localPkgs.sbatch-tui.override {
              sbatchConfig = pkgs.writeText "sbatch.yaml" (
                builtins.toJSON {
                  Program = {
                    VaspCpu.Queue = builtins.map (queue: {
                      Name = queue.name;
                      Recommended = {
                    ...
                    Fdtd.Queue = builtins.map (queue: {
                      Name = queue.name;
                      Recommended = {
                        Cpus =
                          if queue.allocateCpus != null then queue.allocateCpus else queue.mpiThreads * queue.openmpThreads;
                        Memory = queue.memoryGB;
                      };
                    }) slurm.tui.cpuQueues;
                  }
                  // (
                    if slurm.tui.gpuQueues == null then
                      { }
                    else
                      rec {
                        VaspGpu.Queue = builtins.map (queue: {
                          Name = queue.name;
                          Gpu = queue.gpuIds;
                        }) slurm.tui.gpuQueues;
                        Mumax3 = VaspGpu;
