inputs: {
  ...
  config =
    ...
    inputs.lib.mkIf (coturn != null) {
      ...
      nixos = {
        system.sops.secrets."coturn/auth-secret".owner =
          inputs.config.systemd.services.coturn.serviceConfig.User;
        services.acme.cert.${coturn.hostname}.group =
          inputs.config.systemd.services.coturn.serviceConfig.Group;
      };
