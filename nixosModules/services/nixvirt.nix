inputs: {
  options.nixos.services.nixvirt =
    let
                    ...
                    in
                    {
                      ...
                      network = {
                        ...
                        vnc = {
                          port = mkOption {
                            type = types.ints.unsigned;
                            default = 15900 + submoduleInputs.config.network.address;
                          };
                        ...
                        portForward = rec {
                          tcp = mkOption {
                            type = types.listOf (
                              types.submodule {
                                options = rec {
                                  host = mkOption { type = types.ints.unsigned; };
                                  guest = host;
                                };
                              }
                            );
  ...
  config =
    let
      inherit (inputs.config.nixos.services) nixvirt;
    in
    inputs.lib.mkIf (nixvirt != null) {
      assertions = builtins.map (vm: {
                  ...
                  definition =
                    let
                      base = lib.network.templates.bridge {
                        ...
                        subnet_byte = nixvirt.subnet;
                      };
                      host = builtins.map (vm: {
                ...
                inputs.self.inputs.nixvirt.lib.domain.getXML
                  # port from 8bcc23e27a62297254d0e9c87281e650ff777132
                  {
                    ...
                    cputune = inputs.lib.optionalAttrs (v.cpu.set != null) {
                      vcpupin = builtins.genList (cpu: {
                        vcpu = cpu;
                        cpuset = builtins.elemAt v.cpu.set cpu;
                      }) v.cpu.count;
                    };
                    memory = {
                    ...
                    devices = {
                      ...
                      disk = [
                        ...
                        }
                        {
                          ...
                          source.file = if v.storage.iso == null then "${inputs.self.src.iso.netboot}" else v.storage.iso;
                          target = {
                            dev = "sdc";
                            bus = "sata";
                          };
      ...
      security.wrappers.vm = {
        source =
          let
            vm = inputs.pkgs.localPkgs.vm.override {
              vmConfig = inputs.pkgs.writeText "vm.yaml" (
                builtins.toJSON ({
                  virsh = "${inputs.pkgs.libvirt}/bin/virsh";
                  vm =
                    let
                      vms = builtins.groupBy (vm: vm.value.owner) (inputs.lib.attrsToList nixvirt.instance);
                    in
                    builtins.listToAttrs (
                      builtins.map (owner: {
                  ...
                  builtins.map (
                    ...
                    builtins.map
                      (
                        ...
                        builtins.map (
                          port:
                          "${protocol} dport ${builtins.toString port.host} fib daddr type local counter dnat ip to "
      ...
      boot.kernelParams =
        let
          cpusets = builtins.concatLists (
            builtins.map (vm: vm.cpu.set) (
              builtins.filter (vm: vm.cpu.set != null) (builtins.attrValues nixvirt.instance)
            )
          );
        in
        inputs.lib.mkIf (cpusets != [ ]) [ "isolcpus=${builtins.concatStringsSep "," cpusets}" ];
    };
}
