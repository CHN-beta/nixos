...
}:
{
  ...
  config =
    let
      inherit (config.nixos.services) misskey;
    in
    {
      systemd = lib.mkMerge (
        builtins.map (instance: {
          services."misskey-${instance.name}" = rec {
            ...
            after = [
              "network.target"
              "redis-misskey-${instance.name}.service"
              "postgresql.service"
            ];
            requires = after;
          ...
          tmpfiles.rules =
            let
              dir = "/var/lib/misskey/${instance.name}/files";
              owner = "misskey-${instance.name}";
            in
            [
              "d ${dir} 0700 ${owner} ${owner}"
              "Z ${dir} - ${owner} ${owner}"
            ];
      ...
      nixos = {
        services = {
          ...
          postgresql.instances = builtins.listToAttrs (
            builtins.map (instance: {
              name = "misskey_${builtins.replaceStrings [ "-" ] [ "_" ] instance.name}";
              value = { };
            }) (lib.attrsToList misskey.instances)
          );
        ...
        system.sops.templates = builtins.listToAttrs (
          builtins.map (instance: {
