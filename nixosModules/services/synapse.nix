...
inputs: {
  options.nixos.services.synapse.instances =
    let
  ...
  config =
    ...
    in
    {
      ...
      systemd = inputs.lib.mkMerge (
        builtins.map (
          instance:
          let
            workdir = "/var/lib/synapse/${instance.name}";
          in
          {
            services."synapse-${instance.name}" =
              ...
              in
              {
                ...
                environment = {
                  LD_PRELOAD = "${inputs.pkgs.jemalloc}/lib/libjemalloc.so";
                  PYTHONMALLOC = "malloc";
                };
                serviceConfig = {
      ...
      nixos = {
        system.sops = inputs.lib.mkMerge (
          builtins.map (instance: {
            templates."synapse/${instance.name}/config.yaml" = {
              ...
              content =
                ...
                in
                builtins.readFile (
                  (inputs.pkgs.formats.yaml { }).generate "${instance.name}.yaml" {
                    ...
                    signing_key_path =
                      inputs.config.nixos.system.sops.secrets."synapse/${instance.name}/signing-key".path;
                    email = {
            ...
            secrets =
              (builtins.listToAttrs (
                builtins.map
                  (secret: {
                    name = "synapse/${instance.name}/${secret}";
                    value = { };
                  })
                  [
                    "coturn"
        ...
        services = {
          postgresql.instances = builtins.listToAttrs (
            builtins.map (instance: {
              name = "synapse_${builtins.replaceStrings [ "-" ] [ "_" ] instance.name}";
              value.initializeFlags = {
                TEMPLATE = "template0";
                LC_CTYPE = "C";
                LC_COLLATE = "C";
              };
          ...
          redis.instances = builtins.listToAttrs (
            builtins.map (instance: {
              name = "synapse-${instance.name}";
              value.port = instance.value.redisPort;
            }) (inputs.lib.attrsToList synapse.instances)
          );
          nginx.https = builtins.listToAttrs (
            builtins.map (
