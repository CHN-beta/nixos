inputs: {
          ...
          types.submodule (submoduleInputs: {
            options = {
              ...
              loadAverage = mkOption {
                type = types.ints.unsigned;
                default = submoduleInputs.config.threads;
              };
            };
          })
        )
  ...
  config =
    ...
    in
    inputs.lib.mkIf (beesd != null) {
      services.beesd.filesystems = inputs.lib.mapAttrs' (
        n: v:
        inputs.lib.nameValuePair (inputs.utils.escapeSystemdPath n) {
          spec = n;
          inherit (v) hashTableSizeMB;
          extraOptions = [
            "--thread-count"
            "${builtins.toString v.threads}"
            "--loadavg-target"
            "${builtins.toString v.loadAverage}"
            "--verbose"
            "4"
          ];
        }
