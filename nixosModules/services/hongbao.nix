...
{
  options.nixos.services.hongbao = lib.mkOption {
    type = lib.types.nullOr (lib.types.submodule { });
    default = null;
  };
  config =
    let
      inherit (config.nixos.services) hongbao;
    in
    lib.mkIf (hongbao != null) {
      nixos.services = {
        ...
        nginx.https = {
          ...
          "hongbao2026.chn.moe".location =
            let
              cppfile = pkgs.writeTextDir "index.cpp.txt" ''
                # include <iostream>
                # include <crystal>
                # include <httplib.h>
                int main()
      ...
      systemd.tmpfiles.rules = [
        "d /srv/hongbao 0700 hongbao hongbao"
        "Z /srv/hongbao - hongbao hongbao"
      ];
    };
}
