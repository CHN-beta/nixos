{ lib, config, ... }:
{
  options.nixos.services.nginx.https = lib.mkOption {
    type = lib.types.attrsOf (
      lib.types.submodule (siteSubmoduleInputs: {
        options = {
          global = {
            ...
            type = lib.types.attrsOf (
              lib.types.submodule {
                options = {
                  ...
                  addToTransparentProxy = lib.mkOption {
                    type = lib.types.bool;
                    default = true;
                  };
                };
                  ...
                  in
                  {
                    ...
                    return = lib.mkOption {
                      type = lib.types.nullOr (
                        lib.types.submodule {
                          options = {
                            return = lib.mkOption { type = lib.types.nonEmptyStr; };
                          };
                        }
                      );
                      default = null;
          ...
          assertions = (
            ...
            ++ (builtins.map
              ...
              (
                builtins.concatLists (
                  builtins.map (
                    site: (lib.mapAttrsToList (n: v: lib.nameValuePair "${site.name} ${n}" v) site.value.location)
        ...
        (
          ...
          let
            sites = lib.mapAttrsToList (
              sn: sv:
              lib.nameValuePair sn {
                inherit (sv) global;
                listens = builtins.attrValues sv.listen;
                locations = lib.mapAttrsToList (
                  ln: lv:
                  lib.nameValuePair ln (
                    let
                      _ = builtins.head (builtins.filter (type: type.value != null) (lib.attrsToList lv));
                    in
                    _.value // { type = _.name; }
                  )
                ) sv.location;
              ...
              builtins.map (site: {
                ...
                value = {
                  ...
                  basicAuthFile = lib.mkIf (site.value.global.detectAuth != null) (
                    let
                      secret = "nginx/templates/detectAuth/${lib.strings.escapeURL site.name}-global";
                    in
                    config.nixos.system.sops.templates.${secret}.path
                  );
                  extraConfig =
                    let
                    ...
                    builtins.concatStringsSep "\n" (
                      builtins.concatLists [
                        (
                          if (builtins.typeOf index == "list") then
                            [ "index ${builtins.concatStringsSep " " index};" ]
                          else if (index == "auto") then
                            [ "autoindex on;" ]
                          else
                            [ ]
                        )
                      ...
                      // {
                        proxy = {
                          ...
                          extraConfig = builtins.concatStringsSep "\n" (
                            (lib.mapAttrsToList (n: v: ''proxy_set_header ${n} "${v}";'') location.value.setHeaders)
                            ++ (lib.optionals (location.value.detectAuth != null || site.value.global.detectAuth != null) [
            ...
            nixos = {
              services = {
                nginx =
                  ...
                  let
                    listens = builtins.filter (listen: listen.value.addToTransparentProxy) (
                      builtins.concatLists (
                        builtins.map (
                          site:
                          builtins.map (listen: {
                  ...
                  in
                  {
                    ...
                    streamProxy.map = builtins.listToAttrs (
                      builtins.map (site: {
                        inherit (site) name;
                        value = {
                          upstream.port =
                            with nginx.global;
                            httpsPort + httpsPortShift.proxyProtocol + (if site.value.http2 then httpsPortShift.http2 else 0);
                          proxyProtocol = true;
                          rewriteHttps = lib.mkDefault false;
                        };
                      }) (builtins.filter (listen: listen.value.proxyProtocol) listens)
                    );
                    http = builtins.listToAttrs (
                      builtins.map (site: {
                        inherit (site) name;
                        value.rewriteHttps = { };
                ...
                acme.cert = builtins.listToAttrs (
                  builtins.map (site: {
                    inherit (site) name;
                    value.group = config.services.nginx.group;
                  }) sites
                );
              };
              system.sops =
                let
                  inherit (lib.strings) escapeURL;
                  detectAuthUsers = builtins.concatLists (
                    builtins.map (
                      site:
                      (
                        (builtins.map (location: {
                        ...
                        ++ (lib.optionals (site.value.global.detectAuth != null) [
                          {
                            name = "${escapeURL site.name}-global";
                            value = site.value.global.detectAuth.users;
                          }
                        ])
                      )
                    ) sites
                ...
                in
                {
                  templates =
                    ...
                    builtins.listToAttrs (
                      builtins.map (
                        detectAuth:
                        lib.nameValuePair "nginx/templates/detectAuth/${detectAuth.name}" {
                          owner = config.users.users.nginx.name;
                          content = builtins.concatStringsSep "\n" (
                            builtins.map (user: "${user}:{PLAIN}${placeholder."nginx/detectAuth/${user}"}") detectAuth.value
                          );
                        }
                      ) detectAuthUsers
                    );
                  secrets = builtins.listToAttrs (
                    builtins.map (secret: {
                      name = "nginx/detectAuth/${secret}";
                      value = { };
                    }) (lib.unique (builtins.concatLists (builtins.map (detectAuth: detectAuth.value) detectAuthUsers)))
                  );
                };
            };
          }
