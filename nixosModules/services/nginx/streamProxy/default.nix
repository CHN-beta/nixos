{ lib, config, ... }:
{
  ...
  config =
    ...
    in
    lib.mkIf (nginx.streamProxy.map != { }) {
        ...
        map $ssl_preread_server_name $stream_proxy_backend {
          ${builtins.concatStringsSep "\n    " (
            lib.mapAttrsToList (
              ...
              let
                upstream =
                  if (builtins.typeOf v.upstream == "string") then
                    v.upstream
                  else
                    let
                      port =
                        ...
                        if v.upstream.port == null then
                          ...
                          v.upstream.port;
                    in
                    "${v.upstream.address}:${builtins.toString port}";
              in
              ''"${n}" "${upstream}";''
      ...
      nixos.services.nginx = {
        transparentProxy.map = builtins.listToAttrs (
          ...
          ++ (builtins.map
            (site: {
              inherit (site) name;
              value = with nginx.global; streamPort + streamPortShift.proxyProtocol;
            })
            (
              builtins.filter (
                site: ((site.value.proxyProtocol or false) && (site.value.addToTransparentProxy or true))
