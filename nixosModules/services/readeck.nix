{ lib, config, ... }:
{
  ...
  config =
    let
      inherit (config.nixos.services) readeck;
    in
    lib.mkIf (readeck != null) {
      services.readeck = {
        enable = true;
        settings = {
          server.port = 6778;
      ...
      nixos = {
        system.sops = {
          templates."readeck.env".content =
            let
              inherit (config.nixos.system.sops) placeholder;
            in
            ''
              READECK_SECRET_KEY=${placeholder."readeck/secret_key"}
              READECK_DATABASE_SOURCE=postgres://readeck:${
                placeholder."postgresql/readeck"
              }@127.0.0.1:5432/readeck
