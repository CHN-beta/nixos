inputs: {
  options.nixos.services.hpcstat =
    let
  ...
  config =
    let
      inherit (inputs.config.nixos.services) hpcstat;
    in
    inputs.lib.mkIf (hpcstat != null) {
      systemd =
        let
          scripts =
            let
              ...
              ssh =
                "${inputs.pkgs.openssh}/bin/ssh -i ${key} -o StrictHostKeyChecking=no"
        ...
        in
        {
          ...
          timers = builtins.listToAttrs (
            builtins.map (calender: {
              name = "hpcstat-${calender.name}";
              value = {
                wantedBy = [ "timers.target" ];
                timerConfig = {
                  OnCalendar = calender.value;
                  Unit = "hpcstat-${calender.name}.service";
                };
              };
