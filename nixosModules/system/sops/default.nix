inputs: {
          ...
          types.submodule (submoduleInputs: {
            options = {
              ...
              sopsFile = mkOption {
                ...
                default =
                  let
                    secretFileIndex = inputs.lib.lists.findFirstIndex (x: x) null (
                      builtins.map (
                        file:
                        inputs.lib.hasAttrByPath (inputs.lib.splitString "/" submoduleInputs.config.key) (
                          inputs.pkgs.localPkgs.fromYaml (builtins.readFile file)
                        )
                      ) inputs.config.nixos.system.sops.defaultSopsFile
                    );
                  in
                  if secretFileIndex == null then
                    builtins.abort "No sops file found for ${submoduleInputs.config.key}"
                  else
                    builtins.elemAt inputs.config.nixos.system.sops.defaultSopsFile secretFileIndex;
              };
          ...
          let
            getPath =
              ...
              if builtins.typeOf x == "string" then
                ...
                builtins.concatLists (inputs.lib.mapAttrsToList (n: v: builtins.map (l: [ n ] ++ l) (getPath v)) x)
              # simply ignore list, they are sops metadata
              else if builtins.typeOf x == "list" then
                [ [ ] ]
              else
                builtins.abort "Invalid type for availableKeys, get type ${builtins.typeOf x}";
          in
  ...
  config = {
    ...
    sops = {
      ...
      templates = builtins.mapAttrs (n: v: {
        inherit (v)
          content
          owner
          group
