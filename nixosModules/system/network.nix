  ...
  options.nixos.system.network =
    ...
    in
    {
      ...
      implementation = mkOption {
        ...
        default =
          if inputs.config.nixos.model.variant == "desktop" then "networkmanager" else "systemd-networkd";
      };
    };
  config =
    let
      inherit (inputs.config.nixos.system) network;
    in
    inputs.lib.mkMerge [
      # general config
      {
        ...
        systemd.network = {
          ...
          networks = inputs.lib.mkMerge [
            ...
            (builtins.listToAttrs (
              builtins.map (network: {
                name = "10-${network}";
                value.networkConfig.IPMasquerade = "both";
