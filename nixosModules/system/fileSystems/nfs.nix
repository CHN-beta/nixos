...
{
  options.nixos.system.fileSystems.mount.nfs =
    let
        ...
        types.oneOf [
          ...
          (types.submodule (submoduleInputs: {
            options = {
              ...
              mountBeforeSwitch = mkOption {
                type = types.bool;
                default = true;
              };
              readOnly = mkOption {
                type = types.bool;
                default = !submoduleInputs.config.mountBeforeSwitch;
              };
            };
          }))
        ]
  ...
  config =
    let
      inherit (config.nixos.system.fileSystems.mount) nfs;
    in
    lib.mkIf (nfs != { }) (
      lib.mkMerge [
        {
          fileSystems = lib.mapAttrs' (
            n: v:
            lib.nameValuePair (v.mountPoint or v) {
              device = n;
              fsType = "nfs4";
              ...
              options = builtins.concatLists [
                ...
                (lib.optionals (v.mountBeforeSwitch or true) [
                  "retry=15"
                  "x-systemd.device-timeout=15min"
                ])
                (lib.optionals (!(v.mountBeforeSwitch or true)) [
                  "retry=1"
                  "bg"
          ...
          systemd.mounts = builtins.map (mount: {
