inputs: {
  options.nixos.system.fileSystems.impermanence =
    let
  ...
  config.environment.persistence = inputs.lib.mkMerge [
    # generic settings
    {
      "/nix/persistent" = {
        ...
        files = builtins.map (f: "/etc/ssh/ssh_host_${f}_key") [
          "ed25519"
          "rsa"
        ];
      };
      "/nix/rootfs/current" = {
        ...
        ++ builtins.map (f: "/var/lib/systemd/${f}") [
          "linger"
          "coredump"
          "backlight"
        ];
    ...
    {
      "/nix/rootfs/current".users = builtins.listToAttrs (
        builtins.map (user: {
          name = user;
          value.directories = [
            ".cache"
    ...
    (inputs.lib.mkIf (inputs.config.nixos.model.cluster.nodeType or null == "worker") {
      "/nix/persistent".users = builtins.listToAttrs (
        builtins.map (user: {
          name = user;
          value.directories = [
            ".config"
