...
}:
{
  ...
  options.nixos.system.fileSystems = {
    ...
    resume = lib.mkOption {
      ...
      default =
        let
          inherit (config.nixos.system.fileSystems) swap;
        in
        if lib.length swap == 1 then
          if lib.hasPrefix "/dev/" (lib.head swap) then lib.head swap else null
        else
          null;
    };
  };
  config =
    let
      inherit (config.nixos.system) fileSystems;
    in
    lib.mkMerge [
      # mount.vfat
      {
        fileSystems =
          fileSystems.mount.vfat
          |> lib.mapAttrs' (
            n: v:
            lib.nameValuePair v {
              device = n;
      ...
      {
        fileSystems =
          fileSystems.mount.btrfs
          |> lib.mapAttrsToList (
            device-n: device-v:
            device-v
            |> lib.mapAttrs' (
              subvol-n: subvol-v:
              lib.nameValuePair subvol-v {
                device = device-n;
                fsType = "btrfs";
                options = [
                  "subvol=${subvol-n}"
      ...
      (lib.mkIf (fileSystems.resume != null) {
        boot =
          lib.mkConditional (lib.typeOf fileSystems.resume == "string") { resumeDevice = fileSystems.resume; }
            {
