{ lib, config, ... }:
{
  ...
  config =
    let
      inherit (config.nixos.system) initrd;
    in
    lib.mkMerge [
      {
        boot = {
      ...
      (lib.mkIf (initrd.network != null) {
        ...
        boot = {
          initrd = {
            ...
            systemd.network =
              let
                inherit (config.nixos.system.network.settings) dhcp static bridge;
              in
              let
                networks = lib.unique (
                  dhcp
                  ++ (builtins.attrNames static)
                  ++ (builtins.attrNames bridge)
              ...
              in
              {
                networks = builtins.listToAttrs (
                  builtins.map
                    (network: {
                      name = "10-${network}";
                      value = config.systemd.network.networks."10-${network}";
                    })
                    (
                      builtins.filter (
                        network:
                        if initrd.network.interfaces == null then true else builtins.elem network initrd.network.interfaces
                      ) networks
                    )
                );
                netdevs = builtins.listToAttrs (
                  builtins.map
                    (netdev: {
                      name = "10-${netdev}";
                      value = config.systemd.network.netdevs."10-${netdev}";
                    })
                    (
                      builtins.filter (
                        netdev:
                        if initrd.network.interfaces == null then true else builtins.elem netdev initrd.network.interfaces
                      ) netdevs
                    )
                );
              };
