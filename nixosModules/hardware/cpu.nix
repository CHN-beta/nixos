{ lib, config, pkgs, ... }:
{
  options.nixos.hardware.cpu = lib.mkOption
  {
    ...
    default = let inherit (config.nixos.system.nixpkgs) march; in
      if march == null then null
      else if lib.hasInfix "znver" march then "amd"
      else if (lib.hasInfix "lake" march)
        || (builtins.elem march [ "sandybridge" "silvermont" "haswell" "broadwell" ])
        then "intel"
      else null;
  };
  config = let inherit (config.nixos.hardware) cpu; in lib.mkIf (cpu != null) (lib.mkMerge
  [
    (lib.mkIf (cpu == "intel")
    {
