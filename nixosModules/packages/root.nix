...
}:
{
      ...
      lib.types.submodule {
        options.jupyterKernelDefinition = lib.mkOption {
          ...
          default = rec {
            ...
            logo32 =
              pkgs.runCommand "logo-32x32.png" { }
                "${pkgs.imagemagick}/bin/convert ${logo64} -resize 32x32 $out";
          };
        };
      }
    );
  ...
  config =
    let
      inherit (config.nixos.packages) root;
    in
    lib.mkIf (root != null) {
      environment.systemPackages = [ pkgs.root ];
      nixos.packages =
        let
          jupyterKernel = pkgs.jupyter-kernel.create { definitions.root = root.jupyterKernelDefinition; };
        in
        {
          pythonPackages = [
            (
          ...
          pythonEnvFlags = [
            "--prefix JUPYTER_PATH : ${jupyterKernel}"
            "--suffix NIX_PYTHONPATH : ${pkgs.root}/lib"
          ];
