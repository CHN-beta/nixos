  ...
  options.nixos.packages =
    ...
    in
    {
      python = lib.mkOption {
        ...
        default =
          let
            inherit (config.nixos.packages) pythonPackages pythonEnvFlags;
          in
          (pkgs.python3.withPackages (p: builtins.concatLists (builtins.map (f: f p) pythonPackages)))
          .override
            (prev: {
              makeWrapperArgs = prev.makeWrapperArgs or [ ] ++ pythonEnvFlags;
            });
        readOnly = true;
      };
      ...
      pythonEnvFlags = lib.mkOption {
        type = lib.types.listOf lib.types.nonEmptyStr;
        default = [ ];
      };
      vscodeEnvFlags = lib.mkOption {
        type = lib.types.listOf lib.types.nonEmptyStr;
        default = [ ];
      };
    }
    // (builtins.listToAttrs (
      builtins.map (n: lib.nameValuePair n simpleSubmodule) [
        "vasp"
        "mathematica"
        "lumerical"
        "flatpak"
