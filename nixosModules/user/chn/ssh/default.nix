        ...
        home.file = lib.mkIf config.nixos.model.private (
          ...
          // {
            ".ssh/xmuhk_id_rsa".source =
              homeInputs.config.lib.file.mkOutOfStoreSymlink
                config.nixos.system.sops.secrets."chn/xmuhk".path;
          }
        );
      };
    };
    nixos.system.sops.secrets = lib.mkIf config.nixos.model.private (
      lib.mkMerge [
        (builtins.listToAttrs (
          builtins.map (name: lib.nameValuePair "chn/${name}" { owner = "chn"; }) [
            "rsa"
            "rsa.ppk"
            "ed25519"
            "ed25519_sk"
