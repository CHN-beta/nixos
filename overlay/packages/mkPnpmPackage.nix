    ...
    packages = lib.mapAttrs (
      ...
      if value.resolution ? tarball then
        if lib.hasPrefix "https://codeload.github.com" value.resolution.tarball then
          let
            match = lib.strings.match "https://codeload.github.com/([^/]+)/([^/]+)/tar\\.gz/([a-f0-9]+)" value.resolution.tarball;
            repo = fetchGit {
          ...
          in
          value // { resolution.tarball = "file:${tarball}"; }
        else
          let
            tarball = fetchurl rec {
              url = value.resolution.tarball;
              sha256 = extraIntegritySha256.${url};
            };
          in
          value // { resolution.tarball = "file:${tarball}"; }
        ...
        let
          nameAtVersion = builtins.head (lib.splitString "(" name);
        in
        let
          ...
          tarball = fetchurl {
            inherit url;
            sha512 = value.resolution.integrity;
          };
        in
        value // { resolution.tarball = "file:${tarball}"; }
      else
        throw "could not determine source ${name}"
