inputs:
  let
    ...
    bugs =
    {
      ...
      plasma.nixpkgs.overlays = [(final: prev: { kdePackages = prev.kdePackages.overrideScope (final: prev: {
        power-profiles-daemon = prev.power-profiles-daemon.overrideAttrs (prev: {
          patches = prev.patches ++ [ ./plasma.patch ];
        });});})];
    };
  in
    {
