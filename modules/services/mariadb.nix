{ lib, config, pkgs, ... }:
{
  ...
  {
    enable = lib.mkOption { type = lib.types.bool; default = config.nixos.services.mariadb.instances != {}; };
    instances = lib.mkOption
    {
      type = lib.types.attrsOf (lib.types.submodule (submoduleInputs: { options =
      {
        database = lib.mkOption { type = lib.types.nonEmptyStr; default = submoduleInputs.config._module.args.name; };
        user = lib.mkOption { type = lib.types.nonEmptyStr; default = submoduleInputs.config._module.args.name; };
        passwordFile = lib.mkOption { type = lib.types.nullOr lib.types.nonEmptyStr; default = null; };
      };}));
      default = {};
    };
    mountFrom = lib.mkOption { type = lib.types.nullOr lib.types.nonEmptyStr; default = null; };
  };
  config = let inherit (config.nixos.services) mariadb; in lib.mkIf mariadb.enable
  {
    services =
    {
      mysql =
    ...
    systemd.services.mysql.postStart = lib.mkAfter (builtins.concatStringsSep "\n" (builtins.map
      (db:
        let
          passwordFile =
            if db.value.passwordFile or null != null then db.value.passwordFile
            else config.nixos.system.sops.secrets."mariadb/${db.value.user}".path;
    ...
    nixos.system.sops.secrets = builtins.listToAttrs (builtins.map
      (db: { name = "mariadb/${db.value.user}"; value.owner = config.users.users.mysql.name; })
