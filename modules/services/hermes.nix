{ lib, config, pkgs, ... }:
{
  options.nixos.services.hermes = lib.mkOption { type = lib.types.nullOr (lib.types.submodule {}); default = null; };
  config = let inherit (config.nixos.services) hermes; in lib.mkIf (hermes != null)
  {
    ...
    nixos.system.sops =
    {
      ...
      secrets = { "hermes/gemini" = {}; "hermes/tgbot" = {}; "telegram/user/chn" = {}; };
    };
    users =
    {
      users.hermes =
