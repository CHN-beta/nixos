...
{
  # enp4s0 对应 lan1 更靠近 hdmi 口，enp3s0 对应 lan2 更远离 hdmi 口
  config = lib.mkMerge [
    ...
    # 2. 应用 NAT、VLAN 接口及路由策略
    {
      ...
      nixos = {
        system.network.settings = {
          bridge.nixvirt.interfaces = [ "enp4s0.10" ];
          static = {
            nixvirt.ipv4 = {
              ip = "192.168.2.1";
              mask = 24;
        ...
        services.xray.client.v2ray-forwarder.asRouter = [ "enp4s0.20" ];
      };
      networking.nftables = {
        preCheckRuleset = ''
          ...
          hairpin = {
            ...
            content = ''
              chain prerouting {
                ...
                iifname "enp4s0.20" fib daddr type local ip daddr != 192.168.3.0/24 dnat ip to 192.168.3.1
              }
              chain postrouting {
                type nat hook postrouting priority srcnat; policy accept;
                iifname "nixvirt" ip daddr 192.168.2.0/24 masquerade
                iifname "enp4s0.20" ip daddr 192.168.3.0/24 masquerade
              }
            '';
          };
        };
      ...
      services.dnsmasq = {
        ...
        settings = {
          ...
          dhcp-range = [
            ...
            "interface:enp4s0.20,192.168.3.100,192.168.3.250,255.255.255.0,12h"
          ];
          dhcp-host = [
          ...
          dhcp-option = [
            ...
            "interface:enp4s0.20,option:dns-server,192.168.3.1"
          ];
        };
      };
      # otherwise coredns may start before interface up
      systemd.services.coredns = {
        after = [ "sys-subsystem-net-devices-enp4s0.20.device" ];
        bindsTo = [ "sys-subsystem-net-devices-enp4s0.20.device" ];
      };
    }
  ];
}
