...
let
  ...
  deploy =
    let
      ...
      deployKdl =
        ...
        |> lib.map (
          host:
          let
            connect = if lib.elem host doNotUseTailscale then host else "ts.${host}";
          in
          ''
            tab name="${host}" {
    ...
    in
    pkgs.writeShellScript "deploy" "zellij --layout ${deployKdl}";
in
{
  inherit (self.packages.x86_64-linux) dns-push;
