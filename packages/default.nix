...
rec {
  ...
  xmuhk = import ./xmuhk {
    inherit (self) inputs;
    inherit (self.lib) buildNixpkgsConfig;
  };
    ...
    let
      getDrv =
        ...
        if lib.isDerivation x then
          ...
          builtins.concatMap getDrv (builtins.attrValues x)
        else if builtins.isList x then
          builtins.concatMap getDrv x
        else
          [ ];
    in
    pkgs.writeText "src" (builtins.concatStringsSep "\n" (getDrv self.outputs.src));
  ...
  archive =
    let
      ...
      inputListFile =
        self.inputs |> builtins.attrValues |> builtins.concatStringsSep "\n" |> pkgs.writeText "input-list";
    in
